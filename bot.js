...
function cleanMemory() {
    ...
    if (keys.length > 800) {
        ...
        console.log(`[cleanMemory] حُذف ${toDelete.length} جلسة قديمة`);
    }
}
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                if (response.status === 401) {
                    console.error('[Mistral] ❌ خطأ 401: API Key غير صالح!');
                    notifyAdmin('⚠️ تنبيه: Mistral API Key غير صالح (401).');
                    throw new Error('AUTH_ERROR');
                }

                if (response.status === 402) {
                    console.error('[Mistral] ❌ خطأ 402: نفاد رصيد Mistral API!');
                    notifyAdmin('⚠️ تنبيه: نفاد رصيد Mistral API (402).');
...
// ============================================================
// VOXTRAL (Mistral) — تحويل الصوت إلى نص والرد عليه (تم الإصلاح)
// ============================================================
async function transcribeAndReplyAudio(buffer, mimeType, userQuestion, userName, chatHistory) {
    ...
    try {
        ...
        if (e.message === 'AUTH_ERROR') return 'عذراً، مشكلة في إعدادات الخدمة. تم إشعار الأدمن.';
        if (e.message === 'QUOTA_ERROR') return 'عذراً، نفاد رصيد الخدمة مؤقتاً.';
        return 'عذراً، حدث خطأ في معالجة الرسالة الصوتية. حاول كتابة رسالتك نصياً.';
    } finally {
        release();
    }
}
...
// ============================================================
// QR WEB SERVER (Dashboard حقيقي ومتكامل - تم الإصلاح)
// ============================================================
                        ...
                        if (!num || isNaN(limit) || limit < 0) {
                            ...
                            if (sock && isConnected) {
                                try {
                                    ...
                                    await sock.sendMessage(`${num}@s.whatsapp.net`, {
                                        text: `🎉 ${name ? `أهلاً ${name}، ` : ''}تم تعديل حد رسائلك اليومي إلى *${limit}* رسالة!\n\nاكتب *!رصيد* لمعرفة رصيدك الحالي.`
                                    });
                                    result.notified = true;
                                } catch (e) {
                                    result.notified = false;
...
<div class="container">
  ...
  <div class="panel" id="panel-qr">
    <div class="card">
      ...
      <div class="qr-wrap" id="qr-section"><div style="color:#64748b;font-size:14px">جاري التحميل...</div></div>
    </div>
  </div>
  ...
  <div class="panel" id="panel-userstats">
    <div class="card">
      <div class="ch"><h3>📈 إحصائيات المستخدمين التفصيلية</h3></div>
      <table><thead><tr><th>الرقم</th><th>الاسم</th><th>💬 رسائل كلية</th><th>🖼️ صور</th><th>📄 ملفات</th><th>📅 آخر نشاط</th><th>الحد اليومي</th></tr></thead>
  ...
  <div class="panel" id="panel-users">
    <div class="card">
      <div class="ch"><h3>👥 المستخدمون</h3></div>
      <table><thead><tr><th>الرقم</th><th>الاسم</th><th>النوع</th><th>إجراء</th></tr></thead>
  ...
  <div class="panel" id="panel-limits">
    <div class="card">
      <div class="ch"><h3>🔢 حدود الرسائل اليومية</h3></div>
      <div style="padding:14px 18px;background:#0f172a;border-bottom:1px solid #334155;display:flex;gap:8px;flex-wrap:wrap">
        <input class="inp" id="limit-num" placeholder="رقم الهاتف" style="width:180px">
        <input class="inp" id="limit-val" placeholder="عدد الرسائل" type="number" style="width:130px">
        <button class="btn btn-b" onclick="setUserLimit()">✅ تعيين</button>
        <button class="btn btn-y" onclick="resetUserLimit()">↩️ إعادة للافتراضي</button>
      </div>
      <table><thead><tr><th>الرقم</th><th>الاسم</th><th>الحد المخصص</th><th>إجراء</th></tr></thead>
  ...
  <div class="panel" id="panel-vip">
    <div class="card">
      ...
      <table><thead><tr><th>الرقم</th><th>الاسم</th><th>إجراء</th></tr></thead>
      <tbody id="vip-table"><tr><td colspan="3" class="empty">جاري التحميل...</td></tr></tbody>
      </table>
      <div class="form-row"><input class="inp" id="new-vip" placeholder="أدخل رقم الهاتف"><button class="btn btn-g" onclick="addVip()">+ إضافة</button></div>
    </div>
  </div>
  ...
  <div class="panel" id="panel-blacklist">
    <div class="card">
      ...
      <div style="padding:14px 18px;background:#0f172a;display:flex;gap:8px;flex-wrap:wrap">
        <input class="inp" id="bl-num" placeholder="رقم الهاتف" style="width:200px">
        <button class="btn btn-r" onclick="addBlacklist()">⛔ حظر</button>
      </div>
      <table><thead><tr><th>الرقم</th><th>الاسم</th><th>إجراء</th></tr></thead>
  ...
  <div class="panel" id="panel-reports">
    <div class="card">
      <div class="ch"><h3>🚨 البلاغات</h3><button class="btn btn-r" onclick="clearReports()">مسح الكل</button></div>
      <table><thead><tr><th>#</th><th>الاسم</th><th>الرقم</th><th>المشكلة</th><th>الوقت</th></tr></thead>
...
let _data=null;
function showTab(name){const tabs=['overview','qr','broadcast','users','userstats','limits','vip','blacklist','reports'];document.querySelectorAll('.tab').forEach((t,i)=>{t.classList.toggle('active',tabs[i]===name);});document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));document.getElementById('panel-'+name).classList.add('active');loadData();}
function toast(msg,color){const t=document.getElementById('toast');t.textContent=msg;t.style.background=color||'#22c55e';t.classList.add('show');setTimeout(()=>t.classList.remove('show'),3000);}
async function api(action,data={}){try{const r=await fetch('/api',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,data})});return await r.json();}catch(e){toast('خطأ في الاتصال','#dc2626');return{ok:false};}}
async function loadData(){try{const r=await fetch('/data');_data=await r.json();updateUI();}catch(e){console.error(e);}}
function updateUI(){if(!_data)return;const d=_data;document.getElementById('dot').className='dot'+(d.connected?' on':'');document.getElementById('connLabel').textContent=d.connected?'✅ متصل':'❌ غير متصل';const s=d.stats;document.getElementById('s-users').textContent=s.users;document.getElementById('s-active').textContent=s.active;document.getElementById('s-vip').textContent=s.vip;document.getElementById('s-msgs').textContent=s.messages;document.getElementById('s-imgs').textContent=s.images;document.getElementById('s-docs').textContent=s.docs;document.getElementById('s-med').textContent=s.medical;document.getElementById('s-rep').textContent=s.reports;const bc=document.getElementById('broadcast-count');if(bc)bc.textContent='سيصل لـ '+(s.users-1)+' مستخدم';const qrSec=document.getElementById('qr-section');if(d.connected){qrSec.innerHTML='<div class="connected-badge">✅ البوت متصل بواتساب وشغال!</div>';}else if(d.hasQR){qrSec.innerHTML='<img src="/qr-image?t='+Date.now()+'" alt="QR"><div class="qr-steps"><div><span>1.</span> افتح واتساب</div><div><span>2.</span> الإعدادات ← الأجهزة المرتبطة</div><div><span>3.</span> اضغط "ربط جهاز"</div><div><span>4.</span> امسح الكود</div></div>';}else{qrSec.innerHTML='<div style="color:#64748b">⏳ في انتظار رمز QR...</div>';}
renderLimits(d);renderVip(d);renderReports(d);renderUsers(d);renderUserStats(d);}
function renderUserStats(d){const uStats=d.userStats||{},limits=d.userLimits||{};const allUsers=(d.welcomedUsers||[]).map(num=>({num,name:d.userNames[num]||'—',totalSent:(uStats[num]?.totalSent||0),totalImages:(uStats[num]?.totalImages||0),totalDocs:(uStats[num]?.totalDocs||0),lastSeen:uStats[num]?.lastSeen||0,limit:limits[num]!=null?limits[num]:(d.defaultLimit||20),isVip:d.vipNumbers.includes(num)})).sort((a,b)=>b.totalSent-a.totalSent);const tb=document.getElementById('userstats-table');if(!allUsers.length){tb.innerHTML='<tr><td colspan="7" class="empty">لا توجد بيانات</td></tr>';return;}
tb.innerHTML=allUsers.slice(0,200).map(u=>'<tr><td dir="ltr">'+esc(u.num)+'</td><td>'+esc(u.name)+'</td><td style="text-align:center;color:#38bdf8">'+u.totalSent+'</td><td style="text-align:center">'+u.totalImages+'</td><td style="text-align:center">'+u.totalDocs+'</td><td style="font-size:11px;color:#94a3b8">'+(u.lastSeen?new Date(u.lastSeen).toLocaleString('ar-SA'):'—')+'</td><td>'+(u.isVip?'<span class="badge badge-b">♾️ VIP</span>':'<span class="badge badge-g">'+u.limit+'/يوم</span>')+'</td></tr>').join('');}
function renderUsers(d){const allUsers=(d.welcomedUsers||[]).map(num=>({num,name:d.userNames[num]||'—',isVip:d.vipNumbers.includes(num),isBlocked:(d.blacklist||[]).includes(num)}));const tb=document.getElementById('users-table');if(!allUsers.length){tb.innerHTML='<tr><td colspan="4" class="empty">لا يوجد مستخدمون</td></tr>';return;}
tb.innerHTML=allUsers.slice(0,100).map(u=>{const vipBtn=u.isVip?'<button class="btn btn-y" onclick="removeVipNum(`'+u.num+'`)">إزالة VIP</button>':'<button class="btn btn-b" onclick="addVipNum(`'+u.num+'`)">+ VIP</button>';const blockBtn=u.isBlocked?'<button class="btn btn-g" onclick="removeBlacklistNum(`'+u.num+'`)">✅ رفع حظر</button>':'<button class="btn" style="background:#7c3aed;color:#fff" onclick="blockUser(`'+u.num+'`)">⛔ حظر</button>';const badge=u.isBlocked?'<span class="badge badge-r">⛔ محظور</span>':(u.isVip?'<span class="badge badge-b">⭐ VIP</span>':'<span class="badge badge-g">عادي</span>');return '<tr><td dir="ltr">'+esc(u.num)+'</td><td>'+esc(u.name)+'</td><td>'+badge+'</td><td style="display:flex;gap:6px;flex-wrap:wrap">'+vipBtn+'<button class="btn btn-y" onclick="quickSetLimit(`'+u.num+'`)">🔢 حد</button>'+blockBtn+'<button class="btn btn-r" onclick="deleteUser(`'+u.num+'`)">حذف</button></td></tr>';}).join('');}
function renderVip(d){const tb=document.getElementById('vip-table');if(!d.vipNumbers.length){tb.innerHTML='<tr><td colspan="3" class="empty">لا يوجد أرقام VIP</td></tr>';return;}
tb.innerHTML=d.vipNumbers.map(num=>'<tr><td dir="ltr">'+esc(num)+'</td><td>'+esc(d.userNames[num]||'—')+'</td><td><button class="btn btn-r" onclick="removeVipNum(`'+num+'`)">حذف</button></td></tr>').join('');}
function renderReports(d){const tb=document.getElementById('reports-table');if(!d.reports.length){tb.innerHTML='<tr><td colspan="5" class="empty">لا يوجد بلاغات</td></tr>';return;}
tb.innerHTML=d.reports.map((r,i)=>'<tr><td>'+(i+1)+'</td><td>'+esc(r.name||'—')+'</td><td dir="ltr">'+esc(r.sender)+'</td><td>'+esc(r.text)+'</td><td style="font-size:11px">'+esc(r.time)+'</td></tr>').join('');}
function renderLimits(d){const tb=document.getElementById('limits-table');const limits=d.userLimits||{};const keys=Object.keys(limits);if(!keys.length){tb.innerHTML='<tr><td colspan="4" class="empty">لا يوجد حدود مخصصة</td></tr>';return;}
tb.innerHTML=keys.map(num=>'<tr><td dir="ltr">'+esc(num)+'</td><td>'+esc(d.userNames[num]||'—')+'</td><td><span class="badge badge-b">'+limits[num]+' رسالة/يوم</span></td><td><button class="btn btn-y" onclick="resetUserLimitNum(`'+num+'`)">↩️ إعادة</button></td></tr>').join('');}
async function setUserLimit(){const num=document.getElementById('limit-num').value.replace(/\\D/g,'');const limit=parseInt(document.getElementById('limit-val').value,10);if(!num){toast('أدخل رقم الهاتف','#f59e0b');return;}
if(isNaN(limit)||limit<0){toast('أدخل عدداً صحيحاً','#f59e0b');return;}
const r=await api('setUserLimit',{num,limit});if(r.ok){toast('✅ تم تعيين الحد',r.notified?'#22c55e':'#f59e0b');document.getElementById('limit-num').value='';document.getElementById('limit-val').value='';loadData();}else toast(r.msg||'فشل','#dc2626');}
async function resetUserLimit(){const num=document.getElementById('limit-num').value.replace(/\\D/g,'');if(!num){toast('أدخل رقم الهاتف','#f59e0b');return;}
const r=await api('resetUserLimit',{num});if(r.ok){toast('تم الإعادة للافتراضي');loadData();}}
async function resetUserLimitNum(num){const r=await api('resetUserLimit',{num});if(r.ok){toast('تم الإعادة');loadData();}}
async function addVip(){const num=document.getElementById('new-vip').value.replace(/\\D/g,'');if(!num){toast('أدخل رقماً','#f59e0b');return;}
const r=await api('addVip',{num});if(r.ok){toast('✅ تم إضافة VIP');document.getElementById('new-vip').value='';loadData();}}
async function addVipNum(num){const r=await api('addVip',{num});if(r.ok){toast('✅ تم إضافة VIP');loadData();}}
async function removeVipNum(num){if(!confirm('حذف من VIP؟'))return;const r=await api('removeVip',{num});if(r.ok){toast('تم الحذف');loadData();}}
async function deleteUser(num){if(!confirm('حذف هذا المستخدم نهائياً؟'))return;const r=await api('deleteUser',{num});if(r.ok){toast('تم الحذف');loadData();}}
async function clearReports(){if(!confirm('مسح كل البلاغات؟'))return;const r=await api('clearReports');if(r.ok){toast('تم مسح البلاغات');loadData();}}
async function addBlacklist(){const num=document.getElementById('bl-num').value.replace(/\\D/g,'');if(!num){toast('أدخل رقماً','#f59e0b');return;}
if(!confirm('حظر المستخدم '+num+'؟'))return;const r=await api('addBlacklist',{num});if(r.ok){toast('⛔ تم الحظر','#dc2626');document.getElementById('bl-num').value='';loadData();}else toast(r.msg||'فشل','#dc2626');}
async function removeBlacklistNum(num){if(!confirm('رفع الحظر عن '+num+'؟'))return;const r=await api('removeBlacklist',{num});if(r.ok){toast('✅ تم رفع الحظر');loadData();}}
async function blockUser(num){if(!confirm('حظر المستخدم '+num+'؟'))return;const r=await api('addBlacklist',{num});if(r.ok){toast('⛔ تم الحظر');loadData();}else toast(r.msg||'فشل','#dc2626');}
async function quickSetLimit(num){const val=prompt('عدد الرسائل اليومية للمستخدم '+num+':');if(val===null)return;const limit=parseInt(val,10);if(isNaN(limit)||limit<0){toast('رقم غير صحيح','#f59e0b');return;}
const r=await api('setUserLimit',{num,limit});if(r.ok){toast('✅ تم التعيين');loadData();}else toast(r.msg||'فشل','#dc2626');}
async function doAction(action){if(action==='clearSessions'&&!confirm('مسح كل الجلسات النشطة؟'))return;const r=await api(action);toast(r.msg||'✅ تم');loadData();}
async function sendBroadcast(){const text=document.getElementById('broadcast-text').value.trim();if(!text){toast('اكتب الرسالة أولاً','#f59e0b');return;}
if(!confirm('إرسال لجميع المستخدمين؟'))return;const r=await api('broadcast',{text});if(r.ok){toast('✅ '+(r.msg||'بدأ الإرسال'));document.getElementById('broadcast-text').value='';}else toast(r.msg||'فشل','#dc2626');}
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        try {
            ...
            if (!welcomedUsers[sender]) {
                ...
                saveData();
                if (!isGroup && !isAdmin) {
                    notifyAdmin(`🆕 مستخدم جديد: ${userName || 'غير معروف'} (${sender})`);
                }
                if (!isGroup) {
            ...
            if (msgType === 'imageMessage') {
                if (!checkSpam(sender)) {
                    await reply('⚠️ أرسلت صوراً بشكل متسارع، انتظر ثوانٍ.');
                    return;
                }
                ...
                try {
                    ...
                    if (imgSize > 8 * 1024 * 1024) {
                        await reply(`حجم الصورة كبير جداً (${(imgSize/1024/1024).toFixed(1)}MB). الحد الأقصى 8MB.`);
                        return;
                    }
                    ...
                    if (!buffer || buffer.length === 0) {
                        await reply('لم أتمكن من تنزيل الصورة.');
                        return;
                    }
                    ...
                    userChats[sender].push({ role: 'user', content: body ? `[أرسل صورة: ${body}]` : '[أرسل صورة]' });
                    userChats[sender].push({ role: 'assistant', content: res });
                    await reply(res);
                    await react('✅');
                } catch (e) {
                    console.error('[image]', e.message);
                    await reply('لم أتمكن من تحليل الصورة.');
                    await react('❌');
                }
                return;
            }

            // معالجة ملفات PDF - تم الإصلاح
            if (msgType === 'documentMessage') {
                if (!checkSpam(sender)) {
                    await reply('⚠️ أرسلت ملفات بشكل متسارع، انتظر ثوانٍ.');
                    return;
                }
                ...
                try {
                    ...
                    if (mime !== 'application/pdf') {
                        await reply(`📎 "${fileName}"\nالنوع غير مدعوم حالياً. أرسل الملف بصيغة PDF فقط.`);
                        return;
                    }

                    const fileSize = docMsg?.fileLength || 0;
                    if (fileSize > 5 * 1024 * 1024) {
                        await reply(`حجم الملف كبير جداً (${(fileSize/1024/1024).toFixed(1)}MB). الحد الأقصى 5MB.`);
                        return;
                    }
                    ...
                    if (!buffer || buffer.length === 0) {
                        await reply('لم أتمكن من تنزيل الملف.');
                        return;
                    }

                    // التحقق من أنه PDF حقيقي
                    if (buffer.slice(0, 4).toString('ascii') !== '%PDF') {
                        await reply('الملف ليس PDF صحيحاً.');
                        return;
                    }
                    ...
                    try {
                        ...
                        console.warn('[pdf-parse]', pdfErr.message);
                        // محاولة ثانية بدون options
                        try {
                            const parsed2 = await pdfParse(buffer);
                            docText = (parsed2.text || '').trim();
                        } catch (pdfErr2) {
                            console.error('[pdf-parse] فشل:', pdfErr2.message);
                            docText = '';
                        }
                    }
                    ...
                    if (!docText || docText.length < 50) {
                        ...
                        response = await callMistral({
                            ...
                            messages: [
                                { role: 'system', content: getSystemPrompt() },
                                { role: 'user', content: `الملف "${fileName}" قد يكون مصوّراً. بناءً على اسم الملف: ${caption || 'لخص هذا الملف'}` }
                            ],
                            max_tokens: 1500,
                            temperature: 0.3
                        });
                    } else {
                        response = await askAIWithDoc(docText, caption, userName);
                    }

                    if (!userChats[sender]) userChats[sender] = [];
                    userChats[sender].push({ role: 'user', content: `[أرسل PDF: "${fileName}"] ${caption}` });
                    userChats[sender].push({ role: 'assistant', content: response });
            ...
            if (msgType === 'videoMessage') {
                await react('ℹ️');
                await reply(`عذراً، الفيديوهات غير مدعومة حالياً.\nيمكنك أخذ لقطة شاشة وإرسالها كصورة. 📸`);
                return;
            }

            // معالجة الصوت - تم الإصلاح
            if (msgType === 'audioMessage' || msgType === 'pttMessage') {
                if (!checkSpam(sender)) {
                    await reply('⚠️ أرسلت رسائل بشكل متسارع، انتظر ثوانٍ.');
                    return;
                }
                ...
                if (!isAdmin && !isVIP) {
                    ...
                    if (!quota.allowed) {
                        await reply(`⚠️ وصلت للحد اليومي (${DAILY_MSG_LIMIT} رسالة/يوم). يتجدد رصيدك منتصف الليل.`);
                        return;
                    }
                }
                ...
                try {
                    ...
                    if (!buffer || buffer.length === 0) {
                        await reply('لم أتمكن من تنزيل الرسالة الصوتية.');
                        return;
                    }
                    
                    const res = await transcribeAndReplyAudio(buffer, mime, body, userName, userChats[sender]);
                    
                    if (!userChats[sender]) userChats[sender] = [];
                    userChats[sender].push({ role: 'user', content: body ? `[رسالة صوتية: ${body}]` : '[رسالة صوتية]' });
                    userChats[sender].push({ role: 'assistant', content: res });
            ...
            const isVIP = vipNumbers.includes(sender);
            const handledCmd = await handleUserCommand(body, sender, reply, react, isAdmin, isVIP);
            ...
            if (!isAdmin && !isVIP) {
                ...
                if (!quota.allowed) {
                    await reply(
                        ...
                        `لقد استنفدت الـ ${quota.limit} رسالة لهذا اليوم.\n` +
                        `🔄 يتجدد رصيدك في منتصف الليل\n\n` +
                        `للاشتراك بباقة أكبر تواصل: wa.me/${ADMIN_NUMBER}`
                    );
                    return;
                }
            }
            ...
            const languageInstruction = userLanguages[sender] && userLanguages[sender] !== 'ar' 
                ? `\n\nمهم: أجب على هذا المستخدم بلغة "${userLanguages[sender]}" فقط.` 
                : '';
            ...
            try {
                await sock.sendMessage(msg.key.remoteJid, { text: 'حدث خطأ تقني، يرجى المحاولة مرة أخرى.' }, { quoted: msg });
            } catch {}
        }
    });
}
