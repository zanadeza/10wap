...
// ============================================================
// PERSISTENCE — MongoDB Atlas (مع fallback لـ JSON لو MONGODB_URI مش موجود)
// ============================================================
...
async function connectMongo() {
    if (!process.env.MONGODB_URI) {
        console.warn('⚠️ MONGODB_URI غير معيّن — سيتم الحفظ على ملفات JSON محلية.');
        return false;
    }
    try {
...
async function loadDataFromMongo() {
    ...
    try {
        const doc = await _mongoDB.collection('botdata').findOne({ _id: 'main' });
        if (!doc) return null;
        const { _id, ...data } = doc;
        return data;
    } catch (e) {
        console.error('[MongoDB] فشل تحميل البيانات:', e.message);
        return null;
    }
}
...
function saveData() {
    ...
    _saveTimer = setTimeout(async () => {
        ...
        if (_mongoReady) {
            try {
                await _mongoDB.collection('botdata').updateOne(
                    { _id: 'main' },
                    { $set: payload },
                    { upsert: true }
                );
                ...
                console.error('[MongoDB] فشل حفظ البيانات:', e.message);
            }
        }
        // fallback: حفظ على JSON
        try {
...
async function loadChatHistoryFromMongo() {
    if (!_mongoReady) return;
    try {
        const docs = await _mongoDB.collection('chats').find({}).toArray();
        docs.forEach(doc => {
            const { _id, messages } = doc;
            if (_id && messages) _chatHistory[_id] = messages;
        });
        console.log(`✅ تم تحميل محادثات ${docs.length} مستخدم من MongoDB`);
    } catch (e) {
        console.error('[MongoDB] فشل تحميل المحادثات:', e.message);
    }
}

function saveChatHistory() {
    if (_chatHistSaveTimer) return;
    _chatHistSaveTimer = setTimeout(async () => {
        _chatHistSaveTimer = null;
        // حفظ على MongoDB لو متصل
        if (_mongoReady) {
            try {
                const ops = Object.entries(_chatHistory).map(([sender, messages]) => ({
                    updateOne: {
                        filter: { _id: sender },
                        update: { $set: { messages } },
                        upsert: true
                ...
                if (ops.length > 0) await _mongoDB.collection('chats').bulkWrite(ops);
                return;
            } catch (e) {
                console.error('[MongoDB] فشل حفظ المحادثات:', e.message);
            }
        }
        // fallback: حفظ على JSON
...
<div class="card">
  ...
  <div class="footer">${BOT_NAME} Admin Panel &copy; 2025</div>
    ...
    const server = http.createServer(async (req, res) => {
        ...
        // ============================================================
        if (url === '/dashboard.js' && req.method === 'GET') {
            if (!isValidSession(req)) { res.writeHead(401); res.end(); return; }
            try {
                const jsContent = fs.readFileSync(path.join(__dirname, 'dashboard.js'), 'utf-8');
                res.writeHead(200, { 'Content-Type': 'application/javascript; charset=utf-8' });
                res.end(jsContent);
            } catch (e) {
                res.writeHead(404); res.end('// dashboard.js not found');
            }
            return;
        }
...
async function generatePdfContent(userRequest) {
    ...
    try {
        const r = await callMistral({
            ...
            messages: [
                { role: 'system', content:
                    'You are a document planner. Return ONLY valid JSON, no markdown.\n' +
                    'Format: {"title_en":"...","title_ar":"...","color":"#1a5276","sections":[{"heading_en":"...","heading_ar":"...","type":"text|bullets|table"}],"footer_en":"...","footer_ar":"..."}\n' +
                    'Rules: 5-8 sections depending on topic complexity. type=text for explanations, bullets for lists, table for data. Color: medical=#145a32, anatomy=#1a5276, nursing=#1a5276, science=#1e3a5f, general=#4a235a.'
                },
                { role: 'user', content: 'Plan a comprehensive bilingual document about: ' + userRequest }
            ],
            max_tokens: 1000,
            temperature: 0.1
        });
        const rc = r.replace(/```json|```/g, '').trim();
        outline = JSON.parse(rc.slice(rc.indexOf('{')));
    } catch (e) {
        console.warn('[PDF] outline fallback:', e.message);
        outline = {
            title_en: userRequest, title_ar: userRequest, color: '#1a5276',
            sections: [
                { heading_en: 'Introduction',    heading_ar: 'مقدمة',           type: 'text'    },
                { heading_en: 'Key Points',      heading_ar: 'النقاط الرئيسية', type: 'bullets' },
                { heading_en: 'Details',         heading_ar: 'التفاصيل',        type: 'text'    },
                { heading_en: 'Normal Values',   heading_ar: 'القيم الطبيعية',  type: 'table'   },
                { heading_en: 'Clinical Tips',   heading_ar: 'نصائح سريرية',    type: 'bullets' },
                { heading_en: 'Summary',         heading_ar: 'الخلاصة',         type: 'text'    },
            ],
            footer_en: 'Generated by MedTerm Bot', footer_ar: 'تم الإنشاء بواسطة MedTerm Bot'
        };
    }

    // الخطوة 2: توليد كل قسم منفصلاً
    const filledSections = [];
    for (const sec of (outline.sections || [])) {
        const hEn = sec.heading_en || '';
        const hAr = sec.heading_ar || '';
        ...
        try {
            ...
            if (type === 'table') {
                prompt =
                    'Write bilingual content for section "' + hEn + '" in a document about "' + userRequest + '".\n' +
                    'Return ONLY this JSON (no markdown):\n' +
                    '{"content_en":"2-3 sentences intro with numbers/values","content_ar":"مقدمة 2-3 جمل مع أرقام وقيم","table":{"headers":[{"en":"Column","ar":"عمود"}],"rows":[[{"en":"value","ar":"قيمة"}]]}}\n' +
                    'Table: 6-10 rows, comprehensive data. Every cell must have both en and ar values.';
            } else if (type === 'bullets') {
                prompt =
                    'Write bilingual bullet points for "' + hEn + '" in a document about "' + userRequest + '".\n' +
                    'Return ONLY this JSON (no markdown):\n' +
                    '{"content_en":"2-3 sentence intro","content_ar":"مقدمة 2-3 جمل","items":[{"en":"specific point with value/number","ar":"نقطة محددة مع رقم أو قيمة"}]}\n' +
                    'Include 6-8 specific bullet points. Every item must have both en and ar.';
            } else {
                prompt =
                    'Write bilingual paragraphs for "' + hEn + '" in a document about "' + userRequest + '".\n' +
                    'Return ONLY this JSON (no markdown):\n' +
                    '{"content_en":"4-5 detailed sentences with specific numbers, values, clinical details","content_ar":"4-5 جمل تفصيلية مع أرقام وقيم محددة ومعلومات سريرية"}\n' +
                    'Be thorough and specific. Include normal ranges, thresholds, and clinical pearls.';
            }
            ...
            let sc = r.replace(/```json|```/g, '').trim();
            const si = sc.indexOf('{');
            if (si === -1) throw new Error('no JSON');
            sc = sc.slice(si);
            // تنظيف
            sc = sc
                .replace(/"en"\s*:\s*([,}\]])/g, '"en":""$1')
                .replace(/"ar"\s*:\s*([,}\]])/g, '"ar":""$1')
                .replace(/,\s*([}\]])/g, '$1');

            let secData;
            try { secData = JSON.parse(sc); }
            catch (_) { secData = JSON.parse(repairJson(sc)); }

            filledSections.push({ heading_en: hEn, heading_ar: hAr, ...secData });
            console.log('[PDF] ✅ قسم:', hEn);
        } catch (e) {
            console.warn('[PDF] ⚠️ قسم فشل:', hEn, '-', e.message);
            filledSections.push({ heading_en: hEn, heading_ar: hAr, content_en: '', content_ar: '' });
        }
    }

    return {
        title_en: outline.title_en || userRequest,
        title_ar: outline.title_ar || userRequest,
        color:    outline.color    || '#1a5276',
        sections: filledSections,
        footer_en: outline.footer_en || 'Generated by MedTerm Bot',
        footer_ar: outline.footer_ar || 'تم الإنشاء بواسطة MedTerm Bot'
    };
}
...
function findLastCompleteElement(json) {
    ...
    for (let i = 0; i < json.length; i++) {
        const c = json[i];
        if (c==='{' || c==='[') depth++;
        else if (c==='}' || c===']') { depth--; if (depth<=2) last=i+1; }
    }
    return last;
}
...
async function buildPdfBuffer(data) {
    ...
    const prepAr = (input) => {
        ...
        return text.split('\n').map(line => {
            if (!line.trim()) return '';
            if (!/[\u0600-\u06FF]/.test(line)) return line; // إنجليزي — بدون تعديل
            try {
    ...
    const symF  = (b) => hasSymFont ? (b?'SymBold':'Sym') : (b?'Helvetica-Bold':'Helvetica');
    const arF   = (b) => (fontReady && fs.existsSync(PDF_FONT_PATH)) ? (b?'ArBold':'Ar') : symF(b);

    const C     = data.color || '#1a5276';
    const GOLD  = '#c8a200';
    const DARK  = '#1a252f';
    const MED   = '#2c3e50';
    const LIGHT = '#566573';
    ...
    const doc = new PDFDocument({
        ...
        autoFirstPage: true,
        bufferPages: true,
        info: { Title: str(data.title_en||'Document'), Author: 'MedTerm Bot' }
    });
    const chunks = [];
    ...
    // دالة Y الحالي مع هامش من الأسفل
    const cy = () => doc.y;
    const needPage = (needed=80) => { if (cy() > PH - needed - 50) { doc.addPage(); doc.y = 50; } };

    // ── رأس الصفحة الأولى ──
    doc.rect(0, 0, PW, 120).fill(C);
    doc.rect(0, 118, PW, 6).fill(GOLD);

    // عنوان إنجليزي
    const titEn = str(data.title_en||'Document');
    doc.font(symF(true)).fontSize(20).fillColor('#ffffff')
       .text(titEn, ML, 18, { width: W, align: 'left', lineGap: 2 });

    // عنوان عربي (سطر منفصل تحته)
    const titAr = str(data.title_ar||'');
    if (titAr) {
        const prepped = prepAr(titAr);
        if (prepped) {
            doc.font(arF(true)).fontSize(14).fillColor('#aed6f1')
               .text(prepped, ML, 58, { width: W, align: 'right', lineGap: 2 });
        }
    }

    // تاريخ
    const date = new Date().toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' });
    doc.font(symF(false)).fontSize(8).fillColor('#d5d8dc')
       .text('Generated: ' + date + '  |  EN / AR Bilingual Document', ML, 100, { width: W, align: 'center' });
    ...
    for (let si = 0; si < (data.sections||[]).length; si++) {
        ...
        const hEn = str(sec.heading_en || '');
        const hAr = str(sec.heading_ar || '');
        const cEn = str(sec.content_en || '');
        const cAr = str(sec.content_ar || '');

        // ── رأس القسم ──
        if (hEn || hAr) {
            const sy = cy();
            doc.rect(0, sy, PW, 44).fill('#f0f3f7');
            doc.rect(0, sy, 6, 44).fill(C);
            // رقم القسم
            doc.circle(ML + 16, sy + 22, 14).fill(C);
            doc.font(symF(true)).fontSize(11).fillColor('#fff')
               .text(String(si+1), ML+10, sy+15, { width:14, align:'center' });
            // عنوان EN
            if (hEn) {
                doc.font(symF(true)).fontSize(12).fillColor(C)
                   .text(hEn, ML+36, sy+6, { width: W/2, align:'left' });
            }
            // عنوان AR
            if (hAr) {
                const pAr = prepAr(hAr);
                if (pAr) {
                    doc.font(arF(true)).fontSize(11).fillColor(C)
                       .text(pAr, ML + W/2, sy+9, { width: W/2 - 10, align:'right' });
                }
            }
            doc.y = sy + 50;
        }
        ...
        if (cEn) {
            needPage(60);
            doc.font(symF(false)).fontSize(10.5).fillColor(DARK)
               .text(cEn, ML, cy(), { width: W, align: 'left', lineGap: 3 });
            doc.moveDown(0.3);
        }
        if (cAr) {
            needPage(40);
            const pAr = prepAr(cAr);
            if (pAr) {
                // فاصل خفيف
                const fy = cy();
                doc.save().moveTo(ML+30,fy).lineTo(ML+W-30,fy)
                   .strokeColor(C).strokeOpacity(0.15).lineWidth(0.7).stroke().restore();
                doc.y = fy + 5;
                doc.font(arF(false)).fontSize(10.5).fillColor(MED)
                   .text(pAr, ML, cy(), { width: W, align: 'right', lineGap: 3 });
                doc.moveDown(0.4);
            }
        }
        ...
        if (items.length) {
            ...
            for (let ii = 0; ii < items.length; ii++) {
                ...
                const iy = cy();
                const iH = iAr ? 42 : 24;
                if (ii%2===0) doc.rect(ML, iy, W, iH).fill('#eaf4fb').fillOpacity(0.5);
                doc.fillOpacity(1);
                doc.circle(ML+8, iy+(iAr?10:12), 4).fill(GOLD);

                if (iEn) {
                    doc.font(symF(false)).fontSize(10).fillColor(DARK)
                       .text(iEn, ML+18, iy+4, { width: W-20, align:'left' });
                }
                if (iAr) {
                    const pAr = prepAr(iAr);
                    if (pAr) {
                        doc.font(arF(false)).fontSize(10).fillColor(MED)
                           .text(pAr, ML, iy+(iEn?22:4), { width: W-18, align:'right' });
                    }
                }
                doc.y = iy + iH + 4;
            }
        ...
        if (tbl && tbl.headers && tbl.headers.length) {
            ...
            // رأس الجدول
            const thY = cy();
            doc.rect(ML, thY, W, 34).fill(C).fillOpacity(1);
            headers.forEach((h,ci) => {
                const hEn2 = str(h.en||h||'');
                const hAr2 = str(h.ar||'');
                const cx   = ML + cw*ci;
                if (hEn2) doc.font(symF(true)).fontSize(9).fillColor('#fff')
                             .text(hEn2, cx+2, thY+4, { width:cw-4, align:'center' });
                if (hAr2) {
                    const pAr2 = prepAr(hAr2);
                    if (pAr2) doc.font(arF(false)).fontSize(8).fillColor('#c8e0f0')
                                 .text(pAr2, cx+2, thY+18, { width:cw-4, align:'center' });
                }
            });
            doc.y = thY + 38;
            ...
            rows.forEach((row, ri) => {
                ...
                const ry  = cy();
                const rH  = 30;
                doc.rect(ML,ry,W,rH).fill(ri%2===0?'#eaf4fb':'#ffffff').fillOpacity(0.6);
                doc.save().rect(ML,ry,W,rH).strokeColor('#bdc3c7').strokeOpacity(0.3).lineWidth(0.4).stroke().restore();
                doc.fillOpacity(1);
                row.forEach((cell,ci) => {
                    const cEn2 = str(cell.en||cell||'');
                    const cAr2 = str(cell.ar||'');
                    const cx   = ML + cw*ci;
                    if (cEn2) doc.font(symF(false)).fontSize(9).fillColor(DARK)
                                 .text(cEn2, cx+2, ry+4, { width:cw-4, align:'center' });
                    if (cAr2) {
                        const pAr2 = prepAr(cAr2);
                        if (pAr2) doc.font(arF(false)).fontSize(8).fillColor(MED)
                                     .text(pAr2, cx+2, ry+16, { width:cw-4, align:'center' });
                    }
                });
                doc.y = ry + rH;
            });
        ...
        if (si < (data.sections||[]).length-1) {
            ...
            doc.moveDown(0.3);
            const fy2 = cy();
            doc.save().moveTo(ML,fy2).lineTo(ML+W,fy2)
               .strokeColor(C).strokeOpacity(0.1).lineWidth(1.5).stroke().restore();
            doc.moveDown(0.5);
        }
    }
    ...
    for (let pi = range.start; pi < range.start + range.count; pi++) {
        doc.switchToPage(pi);
        const fy3 = PH - 34;
        doc.rect(0, fy3-4, PW, 38).fill(C).fillOpacity(0.06);
        doc.rect(0, fy3-4, PW, 3).fill(GOLD).fillOpacity(1);
        doc.font(symF(false)).fontSize(8).fillColor(LIGHT).fillOpacity(1)
           .text(footEn + '  |  Page '+(pi+1)+'/'+range.count, ML, fy3+4, { width:W/2, align:'left' });
        const pFootAr = prepAr(footAr);
        if (pFootAr) {
            doc.font(arF(false)).fontSize(8).fillColor(LIGHT)
               .text(pFootAr, ML+W/2, fy3+4, { width:W/2, align:'right' });
        }
    }
...
async function fetchWikipediaImage(searchTerm) {
    try {
        ...
        function isValidImgUrl(url) {
            ...
            if (u.includes('.webp')) return false;
        ...
        for (const r of results) {
            ...
            if (t === termLower) { pageTitle = r.title; break; }
            if (t.startsWith(termLower.split(' ')[0])) { pageTitle = r.title; break; }
        }
        console.log(`[wiki] صفحة: "${pageTitle}"`);

        // الخطوة 2: Wikimedia REST API — يعطي صورة الصفحة الرئيسية مباشرة بـ JPEG
        const restUrl = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(pageTitle.replace(/ /g,'_'))}`;
        const restRes = await fetchWithTimeout(restUrl, {
            headers: { 'User-Agent': 'MedTermBot/1.0 (Educational)' }
        }, 8_000);

        if (restRes.ok) {
            const restData = await restRes.json();
            // originalimage = الصورة الأصلية بدون تحويل
            const original = restData?.originalimage?.source;
            const thumb    = restData?.thumbnail?.source;

            if (original && isValidImgUrl(original)) {
                console.log(`[wiki] ✅ originalimage: ${original}`);
                return { url: original, title: pageTitle };
            }
            if (thumb && isValidImgUrl(thumb)) {
                // نطلب أكبر حجم ممكن
                const bigThumb = thumb.replace(/\/\d+px-/, '/800px-');
                if (isValidImgUrl(bigThumb)) {
                    console.log(`[wiki] ✅ thumbnail: ${bigThumb}`);
                    return { url: bigThumb, title: pageTitle };
                }
            }
            console.warn(`[wiki] ⚠️ REST API أعطى صورة غير صالحة: original=${original} thumb=${thumb}`);
        }

        // الخطوة 3 (fallback): قائمة الصور — نجرب واحدة واحدة
        ...
        console.log(`[wiki] ${candidates.length} صورة مرشحة`);
        ...
        for (const candidate of candidates.slice(0, 8)) {
            ...
            if (!url) continue;
            if (mime.includes('svg') || mime.includes('gif') || mime.includes('webp')) {
                console.warn(`[wiki] تجاوز ${candidate.title} (${mime})`);
                continue;
            }
            if (!isValidImgUrl(url)) {
                console.warn(`[wiki] رابط غير صالح: ${url}`);
                continue;
            }
            console.log(`[wiki] ✅ fallback: ${url}`);
            return { url, title: pageTitle };
        }

        console.warn(`[wiki] ❌ لم تُجد صورة صالحة لـ "${pageTitle}"`);
        return null;
...
// الاتصال بـ MongoDB وتحميل البيانات قبل بدء البوت
(async () => {
    const mongoConnected = await connectMongo();
    if (mongoConnected) {
        // تحميل بيانات البوت من MongoDB
        const mongoData = await loadDataFromMongo();
        if (mongoData) {
            // دمج البيانات المحملة من MongoDB مع المتغيرات الحالية
            if (mongoData.userNames)       userNames       = mongoData.userNames;
            if (mongoData.welcomedUsers)   welcomedUsers   = mongoData.welcomedUsers;
            if (Array.isArray(mongoData.vipNumbers)) vipNumbers = mongoData.vipNumbers;
            if (mongoData.vipExpiry)       vipExpiry       = mongoData.vipExpiry;
            if (Array.isArray(mongoData.reports))    reports    = mongoData.reports.slice(-50);
            ...
            console.log(`✅ تم تحميل بيانات ${Object.keys(welcomedUsers).length} مستخدم من MongoDB`);
        } else {
            // أول تشغيل على MongoDB — نحاول نستورد من ملف JSON المحلي لو موجود
            const localData = loadData();
            if (Object.keys(localData.welcomedUsers || {}).length > 0) {
                console.log(`📦 استيراد ${Object.keys(localData.welcomedUsers).length} مستخدم من JSON محلي لـ MongoDB...`);
                userNames       = localData.userNames       || {};
                welcomedUsers   = localData.welcomedUsers   || {};
                vipNumbers      = localData.vipNumbers      || [];
                vipExpiry       = localData.vipExpiry       || {};
                reports         = (localData.reports        || []).slice(-50);
                userLimits      = localData.userLimits      || {};
                userLimitsUsage = localData.userLimitsUsage || {};
                trialNotified   = localData.trialNotified   || {};
                blacklist       = localData.blacklist       || [];
                userLanguages   = localData.userLanguages   || {};
                stats           = { ...stats, ...(localData.stats || {}) };
                saveData(); // احفظ على MongoDB فوراً
                console.log('✅ تم استيراد البيانات لـ MongoDB بنجاح!');
            }
        }
        // تحميل المحادثات من MongoDB
        await loadChatHistoryFromMongo();
    }
    startWebServer();
    startBot();
