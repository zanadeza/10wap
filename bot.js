...
async function generatePdfContent(userRequest) {
    // ── الخطوة 1: تحديد هيكل المستند أولاً (JSON صغير) ──
    const outlineResult = await callMistral({
        model: 'mistral-large-latest',
        messages: [
            { role: 'system', content:
                'You are a document planner. Return ONLY a compact JSON outline.\n' +
                'Format: {"title_en":"...","title_ar":"...","color":"#145a32","sections":[{"heading_en":"...","heading_ar":"...","type":"text|bullets|table"}],"footer_en":"...","footer_ar":"..."}\n' +
                'Rules: 5-7 sections, color by topic (medical=#145a32, anatomy=#1a5276, science=#1e3a5f, general=#4a235a). Return ONLY JSON.'
            },
            { role: 'user', content: `Plan a comprehensive document about: ${userRequest}` }
        ],
        max_tokens: 800,
        temperature: 0.1
    });
    ...
    let outline;
    try {
        const oc = outlineResult.replace(/```json|```/g, '').trim();
        outline = JSON.parse(oc.slice(oc.indexOf('{')));
    } catch (_) {
        // fallback outline
        outline = {
            title_en: userRequest,
            title_ar: userRequest,
            color: '#1a5276',
            sections: [
                { heading_en: 'Overview',       heading_ar: 'نظرة عامة',    type: 'text'    },
                { heading_en: 'Key Concepts',   heading_ar: 'المفاهيم',      type: 'bullets' },
                { heading_en: 'Clinical Details',heading_ar: 'التفاصيل',     type: 'text'    },
                { heading_en: 'Normal Values',  heading_ar: 'القيم الطبيعية',type: 'table'   },
                { heading_en: 'Practical Tips', heading_ar: 'نصائح عملية',   type: 'bullets' },
            ],
            footer_en: 'Generated by MedTerm Bot',
            footer_ar: 'تم الإنشاء بواسطة MedTerm Bot'
        };
    }
    ...
    for (const sec of outline.sections) {
        try {
            ...
            if (secType === 'table') {
                secPrompt =
                    `Write detailed bilingual content for the section "${sec.heading_en}" from a document about "${userRequest}".\n` +
                    `Return ONLY this JSON: {"content_en":"2-3 sentence intro","content_ar":"مقدمة 2-3 جمل","table":{"headers":[{"en":"Col","ar":"عمود"}],"rows":[[{"en":"val","ar":"قيمة"}]]}}\n` +
                    `Make the table comprehensive (6-10 rows), include all relevant values, ranges, and details. JSON only.`;
            } else if (secType === 'bullets') {
                secPrompt =
                    `Write detailed bilingual bullet points for "${sec.heading_en}" from a document about "${userRequest}".\n` +
                    `Return ONLY this JSON: {"content_en":"2-3 sentence intro","content_ar":"مقدمة 2-3 جمل","items":[{"en":"specific detailed point with values/numbers","ar":"نقطة تفصيلية محددة مع أرقام وقيم"}]}\n` +
                    `Include 6-8 specific, informative bullet points. JSON only.`;
            } else {
                secPrompt =
                    `Write comprehensive bilingual paragraphs for "${sec.heading_en}" from a document about "${userRequest}".\n` +
                    `Return ONLY this JSON: {"content_en":"4-5 detailed sentences with numbers, values, clinical info","content_ar":"4-5 جمل تفصيلية مع أرقام وقيم ومعلومات سريرية"}\n` +
                    `Be thorough and include all important details. JSON only.`;
            }

            const secResult = await callMistral({
                model: 'mistral-large-latest',
            ...
            // تنظيف وتحليل
            let sc = secResult.replace(/```json|```/g, '').trim();
            sc = sc.slice(sc.indexOf('{'));
            sc = sc
            ...
            console.log(`[PDF-Gen] ✅ قسم: ${sec.heading_en}`);
        } catch (secErr) {
            console.warn(`[PDF-Gen] ⚠️ قسم فشل: ${sec.heading_en} — ${secErr.message}`);
            filledSections.push({
                heading_en: sec.heading_en,
                heading_ar: sec.heading_ar,
                content_en: `Content about ${sec.heading_en}.`,
                content_ar:  sec.heading_ar + '.'
            });
        }
    }
...
async function buildPdfBuffer(data) {
    ...
    const shapeAr = (input) => {
        ...
        return text.split('\n').map(line => {
            ...
            try { return reshaper.convertArabic(line).split('').reverse().join(''); }
            catch (_) { return line; }
        }).join('\n');
    };
